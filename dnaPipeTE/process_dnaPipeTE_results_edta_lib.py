...
#python3 process_dnaPipeTE_results_edta_lib.py
...
def main():  
  ...
  conn.execute(f"""drop table if exists histoplasma_dnaPipeTE_edta_lib""")
  conn.execute(f"""create table histoplasma_dnaPipeTE_edta_lib
                   (hdel_id int primary key,
  ...
  conn.execute(f"create index idx_hdeo_sample_id on histoplasma_dnaPipeTE_edta_lib(sample_id)")  
  ...
  with tempfile.NamedTemporaryFile(mode='w') as t: 
    for results_dir in glob.glob('output_edta_lib/*'):
      #get values from name of result file
      sample_id, genome_coverage, sample_number, RM_t = re.match(r'output_edta_lib/(.+)_([\.0-9]+)_(\d+)_([\.0-9]+)', results_dir).groups()     
          ...
          with open(sample_results_file, 'r') as r:  
            for line in r:
              ...
              if contig in aligned_nts:
                ...
                hdel_id += 1
                t.write(f"""{hdel_id}\t{sample_id}\t{genome_coverage}\t{sample_number}\t{RM_t}\t{RM_anno}\t{contig}\t{RM_subclass}\t{RM_class}\t{aligned_nt}\t{lib_start}\t{lib_end}\t{lib_hit_prop}\t{genome_per}\n""")
            
            
    t.flush() 
    os.system(f"""sqlite3 {db_file} ".mode tabs" ".import {t.name} histoplasma_dnaPipeTE_edta_lib" """)
