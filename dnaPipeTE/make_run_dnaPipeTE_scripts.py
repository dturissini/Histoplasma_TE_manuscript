...
#python3 make_run_dnaPipeTE_scripts.py edta_lib_1_5_02
...
def main():  
  ...
  dnaPipeTE_sh_dir = os.path.join(base_dir, 'dnaPipeTE_sh_scripts')
  master_sh_dir = os.path.join(base_dir, 'master_sh_scripts')
  ...
  master_sh_file = os.path.join(master_sh_dir, run_prefix + '_run_dnaPipeTE_commands.sh')
  sbatch_file = os.path.join(base_dir, run_prefix + '_run_dnaPipeTE.sbatch')
  
  threads = 16
  image_file = '/work/users/d/t/dturissi/software/dnaPipeTE/dnapipete.img'
  RM_lib_file = '/nas/longleaf/rhel8/apps/repeatmasker/4.1.5/miniconda3/share/RepeatMasker/Libraries/RepeatMasker.lib'
        ...
        for sample_number in sample_numbers: 
          for RM_t in RM_ts:
            ...
            with open(sample_run_dnaPipeTE_sh_file, 'w') as r:    
              ...
              r.write(f"""python3 dnaPipeTE.py -input {fastq_file} -output {sample_mnt_output_dir} -RM_lib {RM_lib_file} -genome_size {genome_len} -genome_coverage {genome_coverage} -sample_number {sample_number} -RM_t {RM_t} -cpu {threads}\n""")
  ...
  with open(sbatch_file, 'w') as o:
    ...
    o.write(f"#SBATCH -o {log_dir}/run_dnaPipeTE_%A_%a.out\n")
    o.write(f"#SBATCH -e {log_dir}/run_dnaPipeTE_%A_%a.err\n\n")
